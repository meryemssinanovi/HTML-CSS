...
// Napraviti getComments funkciju koja ce prikazati email adrese ljudi koji su komentarisali objavu(random u prvih 50),
// zatim random u drugih pedeset i td.
// Sa obzirom da imamo 500 komentara trebamo vratiti 10 email adresa.
...
const getComments = async () => {
  ...
  const forurthFifty = comments.filter(
  ...
  const first = firstFifty.find(
    (el) => el.id === Math.floor(Math.random() * 10) + 1
  ).email;
...
// console.log(Math.floor(Math.random() * 10) + 1); nacin da dobijemo neki random broj od 1 do 10

const getSpecialPost = async () => {
